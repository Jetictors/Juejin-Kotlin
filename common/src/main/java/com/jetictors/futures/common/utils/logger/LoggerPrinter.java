package com.jetictors.futures.common.utils.logger;
...
final class LoggerPrinter implements Printer {
   ...
   *
   * @param tag is the given string which will be used in KLog
   */
  ...
  @Override public synchronized void log(int priority, String tag, String message, Throwable throwable) {
    ...
    message=logHeaderContent(priority, tag, methodCount)+" : "+message;
  ...
  private int getStackOffset(StackTraceElement[] trace) {
    for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
      ...
      if (!name.equals(LoggerPrinter.class.getName()) && !name.equals(KLog.class.getName())) {
        return --i;
      }
    }
    return -1;
